...
import { HandleDetails } from '../Context/CartContext';

const CartProduct = ({title,ids,description,price,thumbnail}) => {
  const [selectedValue,setSelectedValue] = useState(1);
  const [subTotal,setSubTotal] = useState(price);
  const {handleRemove,setTotalQty,setTotal} = HandleDetails();

  function handleSelected(e){
    let prevQty = Number(selectedValue);
    let currQty = Number(e.target.value);

    setSelectedValue(() => currQty);   
    setSubTotal(()=> currQty*price);

    setTotalQty((pre) => pre + currQty);
    setTotalQty((pre) => pre - prevQty);
    setTotal(pre => pre - (prevQty*price));
    setTotal(pre => pre + (currQty*price))
  }
        ...
        <div className="flex flex-col w-full gap-10 xs:w-2/6">
          <div className="flex flex-row flex-wrap items-end justify-end whitespace-nowrap">
            <span data-name="product-cart-count">
              <select name="count" className="px-2 bg-transparent outline-1 outline" value={selectedValue} onChange={(e) => handleSelected(e)}>
